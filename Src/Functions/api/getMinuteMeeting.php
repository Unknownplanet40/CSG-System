...
try {
    ...
    if ($_SESSION['role'] == 1) {
        $stmt = $conn->prepare("SELECT * FROM minutemeetingdocuments $submitted");
    } else {
        ...
        if ($_SESSION['org_position'] < 4) {
            $stmt = $conn->prepare("SELECT * FROM minutemeetingdocuments WHERE org_code = ? $submitted");
            $stmt->bind_param("s", $_SESSION['org_Code']);
        } else {
            $stmt = $conn->prepare("SELECT * FROM minutemeetingdocuments WHERE org_code = ? AND UUID = ? $submitted");

...
try {
    ...
    if ($_SESSION['role'] == 1) {
        $stmt = $conn->prepare("SELECT * FROM officememorandomdocuments");
    } else {
        ...
        if ($_SESSION['org_position'] < 4) {
            $stmt = $conn->prepare("SELECT * FROM officememorandomdocuments WHERE org_code = ?");
            $stmt->bind_param("s", $_SESSION['org_Code']);
        } else {
            $stmt = $conn->prepare("SELECT * FROM officememorandomdocuments WHERE org_code = ? AND UUID = ?");

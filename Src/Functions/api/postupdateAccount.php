...
try {
    ...
    $iregular = $_POST['perirreg']; // int

    if (empty($perFname) || empty($perLname) || empty($perEmail) || empty($perStudentno) || empty($perContact) || empty($perAge) || empty($perBirthdate) || empty($perOrg) || empty($perPosition) || empty($perUUID)) {
    ...
    if ($isError) {
        ...
        $eventMessage = $perFname . " " . $perLname . " Birthday Celebration on " . date('F j, Y', strtotime($perBirthdate));
        $eventtitle = $perFname . " " . $perLname . " Birthday!";
        $start = date('Y-m-d', strtotime($perBirthdate));
        $end = date('Y-m-d', strtotime($perBirthdate));
        $color = 'bs-cyan';
        $type = 'BDay';
        $isEnded = 0;
        $location = 'Its Fucking Birthday';
        $UUID = $perUUID;

        $stmt = $conn->prepare("INSERT INTO sysevents (UUID, title, eventdesc, location, color, type, start, end, isEnded) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param('ssssssssi', $UUID, $eventtitle, $eventMessage, $location, $color, $type, $start, $end, $isEnded);
        $stmt->execute();
        $stmt->close();
